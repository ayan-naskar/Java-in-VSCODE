...
public class SerpinskySquare extends Frame{
    class Point{
        double x,y;
    ...
    public void paint(Graphics gr){
        ...
        Point c=new Point(0.0,1000.0),d=new Point(1000.0,1000.0);
        // drawLine(a,b,g);
        // drawLine(a,c,g);
        // drawLine(b,c,g);
        ...
        while(true){
            printt(rand,g);
            rand = getRandomPoint(rand,a,b,c,d);
            try{Thread.sleep(0);}
            catch(InterruptedException e){}
        }
    }
    ...
    Point getRandomPoint(Point p, Point a, Point b, Point c, Point d){
        Point select;
        double random = Math.random();
        if(random<0.25) select = a;
        else if(random<0.5) select = b;
        else if(random<0.75)select = c;
        else select = d;
